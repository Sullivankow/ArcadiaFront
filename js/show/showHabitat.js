...
async function fetchHabitats() {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Erreur lors de la récupération des habitats : ${response.status}`);
        }
...
function displayHabitats(habitats) {
    ...
    if (Array.isArray(habitats) && habitats.length > 0) {
        habitats.forEach(habitat => {
            ...
            commentaireHabitatCell.textContent = habitat.commentaire_habitat || "Sans nom";
...
async function editHabitat(habitatId) {
    const newHabitatName = prompt("Entrez le nouveau nom pour cet Habitat (laissez vide pour ne pas modifier) :");
    const newHabitatDescription = prompt("Entrez la nouvelle description pour cet habitat (laissez vide pour ne pas modifier) :");
    const newHabitatCommentaire = prompt("Entrez le nouveau commentaire pour cette habitat (laissez vide pour ne pas modifier) :");
    ...
    try {
        ...
        const response = await fetch(`${apiUrl}/habitat/edit/${habitatId}`, requestOptions);
